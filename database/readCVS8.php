...
if (($handle = fopen("data8.csv", "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            mysqli_query($db_handle,"INSERT INTO emails 
										...
										VALUES  
											('$email', '$data[0]', '$data[19]', 1, '$address_id', '$date', 0);");
		...
		if (phoneNumbervalidation($mobile)) {
			mysqli_query($db_handle,"INSERT INTO mobiles 
										...
										VALUES  
												('$mobile', '$data[0]', '$data[19]', 1, '$address_id', '$date', 0);");
